...
const ValuesSection = () => {
  ...
  // Function to handle the next index (for manual scrolling)
  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % values.length);
  };

  // Function to handle the previous index (for manual scrolling)
  const handlePrev = () => {
    setCurrentIndex(
  ...
  // Automatically change the current index every 3 seconds
  useEffect(() => {
        ...
        >
          ...
          <div className="mb-8 flex justify-center items-center w-20 h-20 rounded-full bg-gray-900 mx-auto">
            ...
            {React.createElement(values[currentIndex].icon, {
              className: "h-10 w-10 text-white", // Adjust icon size here
            })}
          </div>
