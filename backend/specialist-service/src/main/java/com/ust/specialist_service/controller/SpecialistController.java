...
public class SpecialistController {
  ...
  @PutMapping("/{id}")
  public Mono<ResponseEntity<Specialist>> updateSpecialist(@PathVariable ObjectId id,
  ...
  public Mono<ResponseEntity<Specialist>> updateSpecialistByEmail(@PathVariable ObjectId id,
      @RequestBody EmailUpdateDto email) {
  ...
  public Flux<SpecialistDto> getSpecialistsByStatus(@RequestParam(name = "status", required = false) String status) {
    if (status != null && !status.isEmpty()) {
      ...
      if (!isValidStatus(status)) {
        return Flux
            .error(new IllegalArgumentException("Invalid status value. Allowed values: pending, approved, rejected"));
      }
  ...
  public Mono<ResponseEntity<Object>> deleteSpecialistByEmail(@PathVariable String email) {
    return specialistService.deleteSpecialistByEmail(email)
        .then(Mono.just(ResponseEntity.noContent().build())) // Returns Mono<ResponseEntity<Void>> when deletion is
                                                             // successful
        .defaultIfEmpty(ResponseEntity.notFound().build()); // Returns Mono<ResponseEntity<Void>> with 404 when the
