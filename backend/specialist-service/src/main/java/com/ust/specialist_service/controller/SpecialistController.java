...
public class SpecialistController {
  ...
  @PutMapping("/{id}")
  public Mono<ResponseEntity<Specialist>> updateSpecialist(
      @PathVariable ObjectId id, @RequestBody Specialist specialistDetails) {
    return specialistService
        .updateSpecialist(id, specialistDetails)
  ...
  public Mono<ResponseEntity<Void>> deleteSpecialist(@PathVariable ObjectId id) {
    return specialistService.deleteSpecialist(id).map(v -> ResponseEntity.noContent().build());
  }
  ...
  public Mono<ResponseEntity<Specialist>> updateSpecialistByEmail(
    ...
    return specialistService
        ...
        .flatMap(
            existingSpecialist -> {
              ...
              return Mono.just(ResponseEntity.status(HttpStatus.CONFLICT).body((Specialist) null));
            })
        .switchIfEmpty( // If no existing specialist with this email, proceed to update
            specialistService
                .updateSpecialistByEmail(id, email)
                .map(updatedSpecialist -> ResponseEntity.ok(updatedSpecialist))
                .defaultIfEmpty(ResponseEntity.notFound().build()))
        .defaultIfEmpty(
            ResponseEntity.status(HttpStatus.NOT_FOUND)
                .build()); // If no specialist with the ID is found
  }
  ...
  @GetMapping("/status")
  public Flux<SpecialistDto> getSpecialistsByStatus(
      ...
      if (!isValidStatus(status)) {
        return Flux.error(
            new IllegalArgumentException(
                "Invalid status value. Allowed values: pending, approved, rejected"));
      }
  ...
  public Mono<ResponseEntity<Object>> updateSpecialistStatus(
      @PathVariable("id") ObjectId id, @RequestBody StatusUpdateDto statusUpdateDto) {

    if (!isValidStatus(statusUpdateDto.getStatus())) {
      return Mono.just(ResponseEntity.badRequest().build());
    }
    return specialistService
        .updateSpecialistStatus(id, statusUpdateDto.getStatus())
  ...
  public Mono<ResponseEntity<Object>> deleteSpecialistByEmail(@PathVariable String email) {
    return specialistService
        .deleteSpecialistByEmail(email)
        .then(
            Mono.just(
                ResponseEntity.noContent()
                    .build())) // Returns Mono<ResponseEntity<Void>> when deletion is
        // successful
        .defaultIfEmpty(
            ResponseEntity.notFound()
                .build()); // Returns Mono<ResponseEntity<Void>> with 404 when the
    // specialist is not found
  }
}
